...
public class ModMenu
{
    ...
    public ModMenu RegisterItem(MMItemBase menuItem)
    {
        ...
        ModMenuMenuItem? menu = ModMenus.Find(x =>
            x.MenuTitle.ToLower().Equals(m_menuTitle.ToLower())
        );
        if (menu is not null)
        {
        ...
        {
            var newMenu = new ModMenuMenuItem(m_menuTitle);
    ...
    {
        ModMenuMenuItem? menu = ModMenus.Find(x =>
            x.MenuTitle.ToLower().Equals(menuTitle.ToLower())
        );
        if (menu is null)
            return false;
    ...
    public static void RemoveAllOwnedBy(Assembly ass)
    {
        foreach (var menu in ModMenus)
            menu.MenuItems.RemoveAll(item => item.ParentAssembly == ass);
    }
